...
new Event({
  ...
  async run(message) {
    ...
    const lista: string =
      (await db.list.get("list")) ?? "erro ao tentar pegar a lista";

    for (const block of lista) {
      if (message.content.includes(block)) {
        // verificando se a mensagem criada contem alguma item do block/list
    ...
    switch (contador) {
      case 1:
        let mes = message.reply({
          content: `${author} Que feio!!, acha isso bonito!? não repita isso novamente, se não vou ter que tomar medida extras.`, // se caso conter algum item do array. ira apagar a mensagem e vai mandar essa
          allowedMentions: { parse: ["users"], repliedUser: true },
        }); // mencionando o usuario
        setTimeout(async () => {
          (await mes).delete();
        }, 30000);
        break;
      ...
      case 3:
        ...
        motion2!.timeout(
          60 * 60 * 1000,
          "castigo de 1hora, motivo: citando varias palavrão"
        );
        break;
    }

...
new Command({
  ...
  async run(interaction) {
    if (!interaction.member.permissions.has("Administrator")) {
      interaction.reply({
        content: "Infelizmente, Você nao tem permissão para usar esse comando.",
        ephemeral,
      });
    } else {
      let configui = new EmbedBuilder()
        ...
        .setDescription(
          "selecione a opção abaixo, para puder configurar o bot."
        );
      ...
      const row = createRow(
        new StringSelectMenuBuilder({
          ...
          options: [
            ...
            {
              label: "Palavra Bloqueada",
              value: "config/messblock",
              emoji: "❌",
            },
